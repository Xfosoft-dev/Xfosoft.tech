...
const EditRequiredSkills = ({ setValue, requiredSkill }: Props) => {
  const { form, fields, onSubmit, handleAppend, handleRemove, data, isActive } =
    useDynamicUpdateForm(requiredSkill);

  useEffect(() => {
    if (data !== undefined) {
      setValue("requiredSkills", data);
    }
  }, [data]);

  const notify = () => toast.success("Required skills Edited Successfully.");
...
export default EditRequiredSkills;
